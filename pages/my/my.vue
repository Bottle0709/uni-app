<template>
  <view>
      ...
      <!-- <card> -->
        <view class="row">
          <block v-for="(item, index) in nav" :key="index">
            <view
              class="span-6 d-flex flex-column j-center a-center py-3"
              hover-class="bg-light-secondary"
              @tap="navigate(item)"
            >
              <view
                class="icon iconfont line-h"
                :class="item.icon"
    ...
    <uni-list-item
      ...
      otherIcon="icon-VIP"
      title="停车缴费"
      @click="goto('/pages/vehicle/payment')"
    ></uni-list-item>
    <uni-list-item
      showExtraIcon
      otherIconStyle="color:#FCBE2D;"
      otherIcon="icon-huangguan"
      title="报修记录"
    ></uni-list-item>
    <uni-list-item
      showExtraIcon
      otherIconStyle="color:#FA6C5E;"
      otherIcon="icon-service"
      title="充电记录"
    ></uni-list-item>
    <uni-list-item
      showExtraIcon
